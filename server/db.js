...
const config = {
    ...
    database: process.env.DB_DATABASE || 'SistemadeventasdeClimasyMInisplit',
...
export async function connectDB() {
    try {
        if (!pool) {
            console.log('Intentando conectar a SQL Server con config:', {
                user: config.user,
            ...
            console.log('✅ --- Conectado a SQL Server Industrial ---');
            await initializeSchema();
            console.log('✅ --- Esquema de base de datos inicializado ---');
        }
        return pool;
    } catch (err) {
        pool = null; // Reset pool so next call retries
        console.error('❌ Error de conexión a SQL Server:', err.message);
        console.error('\n=== GUÍA DE SOLUCIÓN ===');
        console.error('1. Abre SQL Server Configuration Manager');
        console.error('2. Verifica que SQL Server (MSSQLSERVER) esté corriendo');
        console.error('3. Habilita TCP/IP en SQL Server Network Configuration');
        console.error('4. En SSMS, habilita "SQL Server and Windows Authentication mode"');
        console.error('5. Establece la contraseña del usuario "sa" y habilitalo');
        console.error('========================\n');
        throw err; // Re-throw so API routes can return proper error
    }
}
...
async function initializeSchema() {
    ...
    await request.query(`
        ...
        CREATE TABLE clientes (
            ...
            rfc NVARCHAR(50),
            tipo_cliente NVARCHAR(50) DEFAULT 'Residencial'
        )
    `);
    ...
    await request.query(`
        ...
        CREATE TABLE productos (
            ...
            categoria NVARCHAR(100) DEFAULT 'Industrial',
            sku NVARCHAR(100),
            especificaciones NVARCHAR(MAX),
            imagenes_json NVARCHAR(MAX)
        )
    `);
    ...
    await new sql.Request(pool).query(`
        ...
        CREATE TABLE tecnicos (
            ...
            telefono NVARCHAR(20) NOT NULL,
            numero_id NVARCHAR(50) UNIQUE NOT NULL,
            rfc NVARCHAR(50),
            tipo_instalador NVARCHAR(50) NOT NULL DEFAULT 'Residencial',
            usuario_id INT,
    ...
    // Instalaciones
    await new sql.Request(pool).query(`
        IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'instalaciones')
        CREATE TABLE instalaciones (
            ...
            hora_instalacion NVARCHAR(10) NOT NULL,
    ...
    await new sql.Request(pool).query(`
        IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'direcciones_cliente')
        CREATE TABLE direcciones_cliente (
            id INT PRIMARY KEY IDENTITY(1,1),
            cliente_id INT NOT NULL,
            direccion NVARCHAR(MAX) NOT NULL,
            etiqueta NVARCHAR(100) DEFAULT 'Casa',
    ...
    await new sql.Request(pool).query(`
        ...
        CREATE TABLE chats (
            ...
            estado_u1 NVARCHAR(50) DEFAULT 'activo',
            estado_u2 NVARCHAR(50) DEFAULT 'activo',
            fecha_creacion DATETIME DEFAULT GETDATE(),
            ultimo_mensaje DATETIME DEFAULT GETDATE(),
            FOREIGN KEY(instalacion_id) REFERENCES instalaciones(id) ON DELETE NO ACTION,
    ...
    await new sql.Request(pool).query(`
        ...
        ALTER TABLE ventas ADD estado NVARCHAR(50) DEFAULT 'Completada'
    `);
}
...
export const dbOps = {
    ...
    updateEstadoInstalacion: async (id, estado, notas = null) => {
        ...
        return pool.request()
            ...
            .query('UPDATE instalaciones SET estado = @estado, notas = ISNULL(@notas, notas) WHERE id = @id');
    },
        ...
        const result = await pool.query(`
            SELECT t.*,
                (SELECT COUNT(*) FROM instalaciones i 
                 ...
                 AND i.fecha_instalacion = CAST(GETDATE() AS DATE) 
    ...
    addMensaje: async (chat_id, emisor_id, contenido) => {
        ...
        await pool.request().input('cid', sql.Int, chat_id)
            .query('UPDATE chats SET ultimo_mensaje = GETDATE() WHERE id = @cid');
        return { id: result.recordset[0].id };
    },
